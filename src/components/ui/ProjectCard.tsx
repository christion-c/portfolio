      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
            ...
            {project.meta && (
              <span className="rounded-full border border-primary/20 bg-primary/10 px-2.5 py-0.5 text-[11px] font-semibold uppercase tracking-wide text-primary">
                {project.meta}
              </span>
            )}
          </div>
        ...
        <span className="shrink-0 font-display text-3xl font-semibold text-primary/15 transition-colors duration-300 group-hover:text-primary/30">
          {String(index + 1).padStart(2, '0')}
        </span>
      </div>

      <p className="mt-4 text-xs font-semibold uppercase tracking-widest text-primary/70">{project.role}</p>
        ...
        <div className="mt-6 flex gap-5 border-t border-border pt-5 text-sm font-medium">
          {project.liveUrl && (
            <a
              ...
              className="group/link inline-flex items-center gap-1.5 text-text-strong transition-colors hover:text-primary"
            >
              Live site
